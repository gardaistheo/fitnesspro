...
void main() {
  ...
  testWidgets('loads and groups sessions by date with the correct chip label', (
    tester,
  ) async {
  ...
  testWidgets('tapping delete removes the session from the list', (
    tester,
  ) async {
    when(() => mockApiService.get('/workout-sessions')).thenAnswer(
      (_) async => {
  ...
  testWidgets('tapping + Ajouter opens the program picker modal', (
    tester,
  ) async {
    when(() => mockApiService.get('/workout-sessions')).thenAnswer(
      (_) async => {
  ...
  testWidgets('selecting a program opens the date picker before scheduling', (
    tester,
  ) async {
    when(() => mockApiService.get('/workout-sessions')).thenAnswer(
      (_) async => {
  ...
  testWidgets(
    'confirming date and time schedules the session with the picked date',
    (tester) async {
      when(() => mockApiService.get('/workout-sessions')).thenAnswer(
        (_) async => {
          'data': {'data': []},
      ...
      final captured = verify(
        () => mockApiService.post('/workout-sessions', captureAny()),
      ).captured;
  ...
  testWidgets('the date picker does not allow selecting a date before today', (
    tester,
  ) async {
    when(() => mockApiService.get('/workout-sessions')).thenAnswer(
      (_) async => {
  ...
  testWidgets(
    'a backend rejection (e.g. past date) shows an error instead of failing silently',
    (tester) async {
      when(() => mockApiService.get('/workout-sessions')).thenAnswer(
        (_) async => {
          'data': {'data': []},
      ...
      when(() => mockApiService.post(any(), any())).thenThrow(
        Exception('The scheduled date must be today or a future date.'),
      );
      ...
      expect(
        find.text('The scheduled date must be today or a future date.'),
        findsOneWidget,
      );
    },
  );
