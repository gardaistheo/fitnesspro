...
void main() {
  ...
  testWidgets('continue is disabled until a level is selected on step 1', (tester) async {
    await tester.pumpWidget(buildTestable((_) {}));

    expect(find.text('Étape 1 sur 6'), findsOneWidget);
    final button = tester.widget<ElevatedButton>(find.widgetWithText(ElevatedButton, 'Continuer →'));
    expect(button.onPressed, isNull);

    await selectLevel(tester, 'Débutant');

    final buttonAfter = tester.widget<ElevatedButton>(find.widgetWithText(ElevatedButton, 'Continuer →'));
    expect(buttonAfter.onPressed, isNotNull);
  });

  testWidgets('total steps is 6 when location is not Maison, skipping the equipment step', (tester) async {
    QuizData? result;
  ...
  testWidgets('total steps is 7 when location is Maison, including the equipment step', (tester) async {
    await tester.pumpWidget(buildTestable((_) {}));
  ...
  testWidgets('back button returns to the previous step and preserves selections', (tester) async {
    await tester.pumpWidget(buildTestable((_) {}));
  ...
  testWidgets('equipment step "Aucun matériel" link clears all selections', (tester) async {
    await tester.pumpWidget(buildTestable((_) {}));
    ...
    final container = tester.widget<Container>(
      find.ancestor(of: find.text('Haltères'), matching: find.byType(Container)).first,
    );
    ...
    expect((decoration.border as Border).top.color, isNot(equals(const Color(0xFFC1FF4D))));
  });
