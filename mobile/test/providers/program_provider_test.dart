...
void main() {
  ...
  test(
    'addToPlanning posts program_id and scheduled_date and returns true on success',
    () async {
      when(
        () => mockApiService.post('/workout-sessions', any()),
      ).thenAnswer((_) async => {'data': {}});

      final result = await provider.addToPlanning(
        programId: 3,
  ...
  test(
    'addToPlanning strips the "Exception: " prefix so the message is user-presentable',
    () async {
      when(() => mockApiService.post('/workout-sessions', any())).thenThrow(
      ...
      expect(
        provider.error,
        'The scheduled date must be today or a future date.',
      );
    },
  );
}
