...
void main() {
  ...
  test('configure loads customer info and starts with no active entitlement', () async {
  ...
  test('login links the RevenueCat app user id and updates customer info', () async {
    installRevenueCatMocks();
    final provider = SubscriptionProvider();
    await provider.configure();
  ...
  test('logout resets to an anonymous customer info without throwing', () async {
    installRevenueCatMocks();
    final provider = SubscriptionProvider();
    await provider.configure();
    await provider.login('42');
  ...
  test('restorePurchases returns true and isPro becomes true when an entitlement is active', () async {
    installRevenueCatMocks(activeEntitlements: _activeProEntitlement);
    final provider = SubscriptionProvider();
    await provider.configure();
  ...
  test('restorePurchases returns false when no entitlement is active', () async {
    installRevenueCatMocks();
    final provider = SubscriptionProvider();
    await provider.configure();
  ...
  test('refreshCustomerInfo picks up entitlement changes since configure', () async {
    installRevenueCatMocks();
    final provider = SubscriptionProvider();
    await provider.configure();
    expect(provider.isPro, isFalse);
