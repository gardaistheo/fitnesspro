...
void main() {
  ...
  testWidgets('App boots on the Landing screen and can navigate to Dashboard', (
    tester,
  ) async {
  ...
  testWidgets(
    'a restored session boots straight to the dashboard, skipping Landing',
    (tester) async {
      SharedPreferences.setMockInitialValues({});
      final storageService = StorageService();
      await storageService.init();
  ...
  testWidgets(
    'a restored session without an active entitlement boots to the paywall recheck',
    (tester) async {
  ...
  testWidgets(
    'skipping the paywall recheck still lands on the dashboard, not the quiz',
    (tester) async {
      installRevenueCatMocks(paywallResult: 'CANCELLED');
      SharedPreferences.setMockInitialValues({});
      final storageService = StorageService();
  ...
  test('resolveInitialRoute: hasSession + isPro decide the initial route', () {
    ...
    expect(
      resolveInitialRoute(hasSession: false, isPro: true),
      '/',
    ); // no session takes priority
    expect(resolveInitialRoute(hasSession: true, isPro: true), '/dashboard');
    expect(
      resolveInitialRoute(hasSession: true, isPro: false),
      '/paywall-recheck',
    );
  });

  testWidgets(
    'an active entitlement at startup goes straight to the dashboard, no paywall detour',
    (tester) async {
